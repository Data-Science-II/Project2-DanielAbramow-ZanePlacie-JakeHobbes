...
class p1(var x : MatriD, var y : VectoD, var rg : PredictorMat){
 ...
 for(i <- 0 until qoFs.size) { // looping through the qoF measures
  ...
  while(!set.contains(-1)){
    ...
    if(forward_features._1 != -1){
     vec = vec ++ forward_features._2.fit(forward_features._2.fitLabel.indexOf(qoF_name)) //logging new qoF metric
    }
  }
  set.remove(-1)
  println(set)
 ...
 //Forward Selection and plotting for rSqCV
 var set = Set(0)
 var vec = new VectorD(0)
  while(!set.contains(-1)){
 ...
 def belim(): Unit ={
  ...
  for(i <- 0 until qoFs.size) {
   ...
   for(j <- 1 to x.range2.length-1) {
    ...
    vec = vec ++ backward_features._2.fit(backward_features._2.fitLabel.indexOf(qoF_name)) //logging metric value
 ...
 def stepRegression(set : Set[Int], qoF: Int, qoF_name : Any): Tuple2[Set[Int], Double] ={
  ...
  val bqOf = backward_features._2.fit(backward_features._2.fitLabel.indexOf(qoF_name)) //getting qoF value for both forward and backward selection
  val fqOf = forward_features._2.fit(forward_features._2.fitLabel.indexOf(qoF_name))
